...
public interface ScheduleRepository extends JpaRepository<ScreeningSchedule, Integer> {
    ...
    @Query("SELECT s FROM ScreeningSchedule s WHERE s.movie.id = :movieId AND " +
    ...
    @Query("SELECT s FROM ScreeningSchedule s WHERE s.movie.id = :movieId AND s.screeningDate = :screeningDate AND " +
            "(s.screeningDate > :currentDate OR " +
            "(s.screeningDate = :currentDate AND s.startTime >= :currentTime))")
    ...
    @Query("SELECT s FROM ScreeningSchedule s WHERE s.movie.id = :movieId AND s.branch.id = :branchId AND s.screeningDate = :screeningDate AND " +
            "(s.screeningDate > :currentDate OR " +
            "(s.screeningDate = :currentDate AND s.startTime >= :currentTime))")
    ...
    @Query("SELECT DISTINCT s.branch FROM ScreeningSchedule s WHERE s.movie.id = :movieId AND " +
            "(s.screeningDate > :currentDate OR " +
            "(s.screeningDate = :currentDate AND s.startTime >= :currentTime))")

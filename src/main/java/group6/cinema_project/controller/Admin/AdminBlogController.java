...
public class AdminBlogController {
    ...
    public String addBlogPost(@Valid @ModelAttribute("blogPost") BlogPostDto blogPostDto,
        ...
        try {
            // Tự động gán admin đầu tiên trong database làm tác giả
            List<User> users = userRepository.findAll();
            log.info("Tổng số users trong database: {}", users.size());

            if (users.isEmpty()) {
                log.error("Không có user nào trong hệ thống!");
                bindingResult.rejectValue("title", "error.blogPost",
                        "Không có user nào trong hệ thống. Vui lòng tạo user trước.");
                model.addAttribute("tinyMCEUrl", tinyMCEConfig.getTinyMCEUrl());
                return "admin/admin_blog_add";
            }

            // Tìm user có role ADMIN trước, nếu không có thì lấy user đầu tiên
            User selectedUser = users.stream()
                    .filter(user -> "ADMIN".equals(user.getRole()))
                    .findFirst()
                    .orElse(users.get(0));

            blogPostDto.setAuthorId(selectedUser.getId());

            log.info("Đã gán tác giả: {} (ID: {}, Role: {}) cho blog post",
                    selectedUser.getUsername(), selectedUser.getId(), selectedUser.getRole());
        } catch (Exception e) {
